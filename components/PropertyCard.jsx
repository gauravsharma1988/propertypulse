import {
  FaMapMarker,
  FaMoneyBill,
  FaBed,
  FaBath,
  FaRulerCombined,
} from "react-icons/fa";
import Image from "next/image";
import Link from "next/link";

const PropertyCard = ({ property }) => {
  return (
    <div className="rounded-xl shadow-md relative">
      <Image
        src={`/properties/${property.images[0]}`}
        alt={property.name}
        sizes="100vw"
        width={0}
        height={0}
        className="w-full h-auto rounded-t-xl"
      />
      <div className="p-4">
        <div className="text-left md:text-center lg:text-left mb-6">
          <div className="text-gray-600">{property.type}</div>
          <h3 className="text-xl font-bold">{property.name}</h3>
        </div>
        <h3 className="absolute top-[10px] right-[10px] bg-white px-4 py-2 rounded-lg text-blue-500 font-bold text-right md:text-center lg:text-right">
          $
          {property.rates.monthly
            ? `${property.rates.monthly.toLocaleString()}/mo`
            : property.rates.weekly
            ? `${property.rates.weekly.toLocaleString()}/wk`
            : property.rates.nightly
            ? `${property.rates.nightly.toLocaleString()}/nt`
            : ""}
        </h3>

        <div className="flex justify-center gap-4 text-gray-500 mb-4">
          <p>
            <FaBed className=" lg:inline  mr-2" /> {property.beds}
            <span className=" lg:inline">Beds</span>
          </p>
          <p>
            <FaBath className=" lg:inline  mr-2" /> {property.baths}
            <span className=" lg:inline">Baths</span>
          </p>
          <p>
            <FaRulerCombined className=" lg:inline  mr-2" />
            {property.square_feet.toLocaleString()}
            <span className=" lg:inline">sqft</span>
          </p>
        </div>

        <div className="flex justify-center gap-4 text-green-900 text-sm mb-4">
          {property.rates.hasOwnProperty("weekly") && (
            <p>
              <FaMoneyBill className=" lg:inline mr-2" /> Weekly
            </p>
          )}
          {property.rates.hasOwnProperty("monthly") && (
            <p>
              <FaMoneyBill className=" lg:inline mr-2" /> Monthly
            </p>
          )}
          {property.rates.hasOwnProperty("nightly") && (
            <p>
              <FaMoneyBill className=" lg:inline mr-2" /> Nightly
            </p>
          )}
        </div>

        <div className="border border-gray-100 mb-5"></div>

        <div className="flex flex-col lg:flex-row justify-between mb-4">
          <div className="flex align-middle gap-2 mb-4 lg:mb-0">
            <FaMapMarker className="text-lg text-orange-700 mt-1" />
            <span className="text-orange-700">
              {property.location.city} {property.location.state}
            </span>
          </div>
          <Link
            href={`/property/${property._id}`}
            className="h-[36px] bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg text-center text-sm"
          >
            Details
          </Link>
        </div>
      </div>
    </div>
  );
};

export default PropertyCard;
